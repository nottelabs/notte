...
class ActionNodeResolutionPipe:
    ...
    async def forward(
        ...
        if node is None or node.id != action.id:
    ...
    async def fill_node_selectors(self, node: DomNode, snapshot: BrowserSnapshot) -> None:
        ...
        if selectors is None or not selectors.is_valid():
            ...
            selectors = HtmlSelector(
                ...
                notte_selector=None if selectors is None else selectors.notte_selector,
            )
    ...
    async def compute_attributes(
        ...
        selectors = node.computed_attributes.selectors
        if selectors is None or not selectors.is_valid():
            logger.info(f"Filling selectors for node {node.id}")
