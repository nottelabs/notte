...
// Error classes and response helpers stay linkable from guides and API pages.
const supportingClass = name => name === 'Encryption' || name === 'PageFetchResponse' || name === 'SessionFiles' || name.endsWith('Error');
...
const methodOrder = {
  ...
  RemoteFileStorage: ['download', 'downloadBlob', 'upload', 'list', 'metadata', 'stream', 'delete'],
};
...
const hiddenMethods = new Set([
  ...
  'files/delete', 'remotefilestorage/setsessionid', 'remotefilestorage/forsession',
]);
...
export function createReference(root = sdkRoot) {
  ...
  for (const group of groups.sort((a, b) => (classOrder.indexOf(a.group) + 1 || 100) - (classOrder.indexOf(b.group) + 1 || 100))) {
    ...
    if (supportingClass(group.group) || group.group === 'NotteClient') continue;
    const category = group.group === 'NotteClient' ? 'Getting Started'
      : ['NotteVault', 'NottePersona', 'SessionFiles', 'RemoteFileStorage', 'NotteSecrets', 'NotteUsage'].includes(group.group) ? 'Tooling' : 'Core Features';
