...
options=("In browser (recommended)" "Terminal")
...
function select_option {
    ...
    while true; do
        ...
        case `key_input` in
            enter)
                SELECTED_MODE="${options[$selected]}"
                break;;
            up)    ((selected--));
...
echo -e "\n\nSelect option using up/down keys and enter to confirm:\n"
select_option "${options[@]}"

if [ "${SELECTED_MODE}" == "In browser (recommended)" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        ...
        echo "Please open $url or choose the terminal option"
    fi
else
...
curl -s https://raw.githubusercontent.com/nottelabs/notte/branding/examples/quickstart.py -o examples/quickstart.py
