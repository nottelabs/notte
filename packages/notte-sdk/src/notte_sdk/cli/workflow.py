...
workflow_app = typer.Typer(
    name="workflow",
    help="Manage workflow lifecycle (create, update, run, benchmark)",
...
def workflow_callback(
    ...
    workflow_path: Annotated[
        ...
        typer.Option(
            "--workflow-path",
            help="Path to the workflow Python file (required unless running from workflow file)",
        ),
    ] = None,
) -> None:
    """Workflow management commands."""
    # Store workflow_path in context for subcommands to access
    _ = ctx.ensure_object(dict)  # type: ignore[assignment]
    ctx.obj["workflow_path"] = workflow_path
...
def get_workflow_path_from_context(ctx: typer.Context, workflow_path: Path | None) -> Path:
    """
    Get workflow_path from context (parent) or parameter (subcommand), with fallback to auto-detection.

    Priority:
    1. Subcommand --workflow-path parameter (highest priority)
    2. Parent (workflow) --workflow-path option
    3. Auto-detection from sys.argv[0] when running from workflow file

    Raises:
        typer.BadParameter: If no path provided and auto-detection fails
    """
    # Priority 1: subcommand parameter
    if workflow_path is not None:
        return workflow_path.resolve()

    # Priority 2: parent context (from callback)
    if ctx.parent and isinstance(ctx.parent.obj, dict):
        parent_obj: dict[str, Any] = ctx.parent.obj  # type: ignore[assignment]
        parent_path = parent_obj.get("workflow_path")
        if parent_path is not None and isinstance(parent_path, Path):
            return parent_path.resolve()

    # Priority 3: auto-detection
    auto_detected = get_default_file_path()
    if auto_detected is not None:
        return auto_detected

    raise typer.BadParameter("File path is required. Provide it using --workflow-path option.")
...
def get_default_file_path() -> Path | None:
    ...
    # Check for workflow subcommand followed by a command
    # sys.argv format: ['script.py', 'workflow', 'create', ...] or ['script.py', 'create', ...]
    if len(sys.argv) > 1:
        # Handle both "workflow create" and direct "create" (for backward compatibility)
        cmd_index = 1
        if sys.argv[1] == "workflow" and len(sys.argv) > 2:
            cmd_index = 2
        if sys.argv[cmd_index] in ["create", "update", "run", "benchmark"]:
...
def create(
    ...
    resolved_file = get_workflow_path_from_context(ctx, workflow_path)
    logger.info(f"Creating workflow from {resolved_file}")

    # Load the workflow function
    _module, _func_name, func = load_workflow_file(resolved_file)
    ...
    if not name:
        ...
        default_name = resolved_file.stem.replace("_", " ").title()
    ...
    existing_metadata = WorkflowMetadata.from_file(resolved_file)
    ...
    with workflow_file_for_upload(resolved_file) as temp_file:
        ...
        metadata = WorkflowMetadata(
            ...
            author=get_git_author(resolved_file),
            creation_date=workflow_obj.response.created_at.isoformat(),
            last_update_date=workflow_obj.response.updated_at.isoformat(),
        )

        update_metadata_in_file(resolved_file, metadata)

        logger.info(f"Metadata block added to {resolved_file}")
...
def update(
    ...
    resolved_file = get_workflow_path_from_context(ctx, workflow_path)
    logger.info(f"Updating workflow from {resolved_file}")

    # Read metadata
    metadata = get_workflow_metadata(resolved_file, require_id=True)
    ...
    with workflow_file_for_upload(resolved_file) as temp_file:
        ...
        logger.info(f"Metadata updated in {resolved_file}")
...
def run(
    ...
    if local:
        logger.info(f"Running workflow locally from {resolved_file}")
        import subprocess

        result = subprocess.run([sys.executable, str(resolved_file)], check=False)
        raise typer.Exit(result.returncode)

    if not api_key:
        raise typer.BadParameter("NOTTE_API_KEY not found. Set it in environment or use --api-key flag.")

    logger.info(f"Running workflow on cloud from {resolved_file}")
...
def benchmark(
    ...
    if local:
        logger.info(
            f"Running benchmark locally from {resolved_file} ({iterations} iterations, {timeout} min timeout, parallelism={parallelism})"
        )
    else:
        if not api_key:
            raise typer.BadParameter("NOTTE_API_KEY not found. Set it in environment or use --api-key flag.")
        logger.info(
            f"Running benchmark on cloud from {resolved_file} ({iterations} iterations, {timeout} min timeout, parallelism={parallelism})"
        )
        ...
        try:
            if local:
                ...
                result = subprocess.run(
                    [sys.executable, str(resolved_file)],
                    check=False,
                    capture_output=True,
                    text=True,
                )
