...
if TYPE_CHECKING:
    ...
    from notte_sdk._sync.personas import NottePersona
    from notte_sdk._sync.sessions import RemoteSession
    from notte_sdk._sync.vaults import NotteVault
    from notte_sdk._sync.workflows import NotteFunction
...
class AgentsClient(BaseClient):
    ...
    def start(self, **data: Unpack[SdkAgentStartRequestDict]) -> AgentResponse:
        ...
        return self.request(AgentsClient._agent_start_endpoint().with_request(request))
    ...
    def status(self, agent_id: str) -> LegacyAgentStatusResponse:
        """Get agent status."""
        endpoint = AgentsClient._agent_status_endpoint(agent_id=agent_id)
    ...
    def run(self, **data: Unpack[SdkAgentStartRequestDict]) -> AgentStatusResponse:
        ...
        return self.watch_logs_and_wait(agent_id=response.agent_id, session_id=response.session_id)
...
class RemoteAgent:
    ...
    class AgentWorkflow:
        ...
        def __init__(self, client: AgentsClient, agent_id: str) -> None:
    ...
    def __init__(
        ...
        **data: Unpack[AgentCreateRequestDict],
        ...
        """Create a new RemoteAgent instance."""
        if _client is None:
            raise ValueError("AgentsClient is required")
...
class BatchRemoteAgent:
    ...
    def run(
        ...
        **args: Unpack[AgentRunRequestDict],
        ...
        def agent_task() -> AgentStatusResponse:
            with RemoteSession(session_id=self.session.session_id, _client=self.root_client.sessions) as session:
                ...
                _ = agent.start(**args)
                return agent.watch_logs_and_wait(log=False)

        return BatchRemoteAgent.run_batch(agent_task, n_jobs=n_jobs, strategy=strategy)
