...
class WorkflowsClient(BaseClient):
    ...
    def get(self, function_id: str, **data: Unpack[GetFunctionRequestDict]) -> GetFunctionWithLinkResponse:
        ...
        return self.request(self._get_workflow_endpoint(function_id).with_params(params))
    ...
    def run(
        ...
        if not request.stream:
            return self.request(endpoint, timeout=timeout or self.WORKFLOW_RUN_TIMEOUT)
...
class RemoteWorkflow:
    ...
    def _ensure_created(self) -> None:
        ...
        if self._needs_create and self._response is None:
            ...
            self._response = self.client.create(**self._create_data)
    ...
    def fork(self) -> "RemoteWorkflow":
        ...
        self._ensure_created()
        fork_response = self.client.fork(function_id=self.function_id)
    ...
    def delete(self) -> None:
        ...
        self._ensure_created()
        resp = self.response
        _ = self.client.delete(function_id=resp.function_id)
    ...
    def get_url(self, version: str | None = None, decryption_key: str | None = None) -> str:
        ...
        if not isinstance(resp, GetFunctionWithLinkResponse) or version != resp.latest_version:
            self._response = self.client.get(function_id=resp.function_id, version=version)
        ...
        if not decrypted:
            if decryption_key is None:
                raise ValueError(
                    "Decryption key is required to decrypt the function download url. Set the `NotteFunction(function_id='<your-function-id>', decryption_key='<your-key>')` when creating the function."
                )
    ...
    def download(
        ...
        with httpx.Client() as http_client:
            try:
                response = http_client.get(file_url, timeout=30)
...
class NotteFunction(RemoteWorkflow):
    """Async function wrapper (preferred over RemoteWorkflow).
