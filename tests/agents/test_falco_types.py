...
def test_validate_against_observation():
    """Test validation of actions against observation."""
    # Create a valid state
    state = AgentState(
        previous_goal_status="success",
        previous_goal_eval="Goal was achieved",
    ...
    valid_observation = {
        "elements": [
            {"id": "test_id", "type": "button", "text": "Click me"},
        ]
    }
    errors = output.validate_against_observation(valid_observation)
    assert len(errors) == 0

    # Test with invalid observation (missing element)
    invalid_observation = {
        "elements": [
            {"id": "other_id", "type": "button", "text": "Click me"},
        ]
    }
    errors = output.validate_against_observation(invalid_observation)
    assert len(errors) == 1
    assert "non-existent element ID: test_id" in errors[0]

    # Test with empty observation
    empty_observation = {"elements": []}
    errors = output.validate_against_observation(empty_observation)
    assert len(errors) == 1
    assert "non-existent element ID: test_id" in errors[0]

    # Test with observation missing elements key
    no_elements_observation = {}
    errors = output.validate_against_observation(no_elements_observation)
    assert len(errors) == 1
    assert "non-existent element ID: test_id" in errors[0]
