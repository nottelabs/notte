...
def find_snippets_examples(
    ...
    for source in sources:
        ...
        yield from _extract_code_chunks(
            source if isinstance(source, Path) else Path(""), code, group
        )
...
def handle_vault_manual(
    ...
    code = code.replace("<your-mfa-secret>", "JBSWY3DPEHPK3PXP").replace("my_vault_id", "8c618111-1d01-4e54-842a-6cac98ef4838")
    try:
        run_example(eval_example, code=code)
...
def extract_param_fields(content: str) -> list[str]:
    ...
    param_pattern = (
        r'<ParamField[^>]*path="([^"]*)"[^>]*type="([^"]*)"[^>]*>(.*?)</ParamField>'
    )
...
def test_session_parameters_in_sync():
    ...
    if missing_in_session:
        error_messages.append(
            f"Parameters missing in session.mdx: {missing_in_session}"
        )
    if extra_in_session:
