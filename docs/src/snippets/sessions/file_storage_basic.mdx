...
with client.Session(storage=storage) as session:
    ...
    response = agent.run(
        task="Upload the PDF document to the website and download the confirmation receipt",
...
for file_name in downloaded_files:
    success = storage.download(file_name=file_name, local_dir="./results")  # Save to "results" folder
