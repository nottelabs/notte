...
const completed = await new FixtureClient().NotteFunction({ function_id: functionId })
  .run({ url: 'https://example.com' }, { stream: false });
...
assert.equal(runStatus.function_run_id, runId);
