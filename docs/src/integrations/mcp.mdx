...
The hosted server generates its tools directly from the public Notte API, so every capability you can reach over REST is also available as an MCP tool. Tool names and parameters mirror the [API reference](/api-reference/authentication), and new API endpoints become tools automatically.
