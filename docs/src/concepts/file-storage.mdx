...
Before running this example, set `session_id` (Python) or `sessionId` (Node) to
your storage's session ID, and `file_id` or `fileId` to the ID returned by uploading
or listing the file. The Node example uses the `client` initialized above.
