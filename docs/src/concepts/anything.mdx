...
The Node SDK wraps this endpoint as [`client.anything.start()`](/typescript-sdk-reference/notteanything/start). It posts to `api.notte.cc/anything/start`, which proxies to the same v2 backend and pipes the stream back, so you get the parsed events without writing the reader yourself:

```typescript
import { NotteClient } from 'notte-sdk';

const client = new NotteClient();
const run = await client.anything.start({ task: 'fetch the top 3 hacker news posts' });
