...
```python
replay(wait: bool = True, timeout: float = 240.0, poll_interval: float = 5.0) -> ReplayResponse
```

Get presigned URLs for the session's MP4 replay. See [`replay()`](/sdk-reference/remotesession/replay) for full documentation.
