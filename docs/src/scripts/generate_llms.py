...
AGENT_READING_GUIDANCE = [
    ...
    "",
    "You MUST follow the CLI-first browser exploration path. Do not use SDK docs, SDK examples, SDK reference pages, "
    "or SDK code from any source until a CLI browser session is running and the target page has been explored "
    "interactively.",
    "",
    "The CLI path is not a tutorial detour. It gives you live page observations, tested selectors from real browser "
    "interaction, session/auth context, replay/debug data, and workflow code via `notte sessions workflow-code`. "
    "Writing SDK code before CLI exploration means guessing at dynamic selectors, auth state, CAPTCHAs, and anti-bot "
    "behavior.",
    "",
    "## CLI-First Workflow",
    "",
    "Complete this loop before using SDK documentation or writing SDK code:",
    "",
    "```bash",
...
SDK_READING_GATE = [
    ...
    "",
    "STOP. Do not use this SDK section yet unless you have already completed the Quickstart-led CLI workflow.",
    "",
    "Before using any SDK link below, you must have a running CLI browser session, have explored the target page "
    "interactively, and have a concrete workflow or `notte sessions workflow-code` output to refine. If you have not "
    "run `notte sessions start`, go back to https://docs.notte.cc/quickstart.md now.",
    "",
    "Use the SDK only after CLI exploration has produced live observations, tested selectors, session/auth context, "
    "and replay/debug data. SDK-first code is guesswork on real-world pages with dynamic selectors, CAPTCHAs, and "
    "anti-bot behavior.",
    "",
]
