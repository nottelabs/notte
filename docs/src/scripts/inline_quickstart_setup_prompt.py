...
def setup_prompt(skill_docs: str) -> str:
    ...
    sections = [
        ...
        "",
        "Your end goal is a reliable Python SDK automation script. Do not write the initial browser automation from memory.",
        "",
        "Use the Notte skill and CLI as the authoring and validation path: inspect the live site, validate actions and selectors against real browser state, handle auth/session details, then export workflow code with `notte sessions workflow-code`. That exported code is the SDK script you should run, customize, or turn into a deployed Function.",
        "",
        "The intended workflow is generate then edit, not write from scratch.",
        "",
        "## Agent Rule: Do Not Guess Selectors",
        "",
        "Do not write SDK code that includes element IDs, selectors, or form field names unless they came from a live `observe()` call or CLI `notte page observe` output. Scripts written with guessed selectors WILL fail.",
        "",
        ...
        "```",
        "",
        "If authentication is missing, do not stop, do not create a fallback script, and do not ask the user for an API key first. Run the interactive login command:",
        "",
        "```bash",
        "notte auth login",
        "```",
        ...
        "```",
        "",
        "Only continue to browser automation after `notte auth status` succeeds. If login does not complete after 5 minutes, ask the user whether they want to retry login or set `NOTTE_API_KEY`.",
        "",
        "Do not generate placeholder automation, SDK scripts, or README-only instructions merely because Notte is unauthenticated. Authentication is part of the setup workflow.",
        "",
        "## Required CLI-to-SDK Loop",
        "",
        "Complete this loop before using SDK docs or writing SDK code:",
        "",
        "```bash",
