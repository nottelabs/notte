...
<Tip>
  Use Python's `with` statement or TypeScript's `session.use()` to stop sessions even if errors occur.
</Tip>
...
Python's `with` statement and TypeScript's `use()` callback guarantee cleanup:
