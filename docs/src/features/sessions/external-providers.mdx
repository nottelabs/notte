...
Notte can connect to any browser that exposes a Chrome DevTools Protocol (CDP) endpoint. This allows you to use Notte's features (Agents, Actions, Scraping) with browser infrastructure from providers like Kernel.sh, BrowserBase, or your own custom browser setup.
...
<CodeGroup>
  ...
  with notte_client.Session(cdp_url=cdp_url) as session:
...
Connect Notte to your own browser running with remote debugging:

<CodeGroup>
  ```python Python - Local Chrome
  from notte_sdk import NotteClient
  import subprocess
  import time
  ...
  with notte_client.Session(cdp_url=cdp_url) as session:
      ...
      # Use Notte's features
      agent = notte_client.Agent(session=session)
      result = agent.run(task="Extract page title")

  # Clean up
  chrome_process.terminate()
  ```

  ```python Python - Docker Browser
  from notte_sdk import NotteClient
  import docker
  ...
  try:
      # Connect Notte to Docker browser
...
- Live viewer (use provider's viewer if available)
...
| Feature | Notte Built-in | Kernel.sh | BrowserBase | Custom Setup |
|---------|---------------|-----------|-------------|--------------|
| Setup Complexity | Simple | Simple | Medium | Complex |
| Anti-detection | ✅ Built-in | ✅ Built-in | ✅ Built-in | ❌ Manual |
| Captcha Solving | ✅ Optional | ❌ | ❌ | ❌ |
| Residential Proxies | ✅ Optional | ✅ | ✅ | ❌ Manual |
| Session Replay | ✅ Yes | ❌ | ❌ | ❌ |
| Live Viewer | ✅ Yes | ✅ Yes | ✅ Yes | Depends |
| Pricing | Per session | Per session | Per session | Your costs |
...
try:
    ...
    # Use with Notte
    with notte_client.Session(cdp_url=external_browser.cdp_url) as session:
        # Your automation
        pass
...
- **Custom Needs**: Use custom Docker/local setup
- **Simplicity**: Use Notte's built-in browsers
- **Existing Infrastructure**: Connect to your current setup
...
<CardGroup cols={2}>
  ...
  <Card title="CDP Reference" icon="code" href="/features/sessions/cdp">
    Learn more about CDP integration
  </Card>
</CardGroup>
