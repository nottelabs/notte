...
<CodeGroup>
  ```python Python - Default Proxies
  from notte_sdk import NotteClient

  client = NotteClient()

  # Use default residential proxies
  with client.Session(proxies=True) as session:
      page = session.page
      page.goto("https://example.com")
  ```

  ```python Python - Country-Specific
  from notte_sdk import NotteClient
  from notte_sdk.types import NotteProxy, ProxyGeolocation, ProxyGeolocationCountry

  client = NotteClient()

  # Use proxies from a specific country
  proxy = NotteProxy(
...
<Note>
  See the [Proxies Guide](/features/sessions/proxies) for advanced proxy configuration and troubleshooting.
</Note>
...
<Note>
  See the [Captcha Guide](/features/sessions/captchas) for more details on captcha solving.
</Note>
...
Automatically load and save cookies across sessions:
...
<CodeGroup>
  ...
  # Cookies automatically loaded from file at start
  # and saved back to file when session ends
  with client.Session(cookie_file="cookies.json") as session:
      page = session.page
      page.goto("https://example.com")
      # Log in, navigate, etc.
      # Cookies are automatically saved when exiting
  ```
</CodeGroup>
...
<CodeGroup>
  ...
  with client.Session() as session:
      ...
      cookies = session.get_cookies()
      with open("cookies_new.json", "w") as f:
          json.dump(cookies, f)
  ```
</CodeGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="Advanced Stealth" icon="user-secret" href="/features/sessions/proxies">
