...
<CodeGroup>
  ...
  def search_products(query: str, max_results: int = 10):
      with client.Session() as session:
          ...
          session.execute(actions.Fill(selector="input[name='search']", value=query))
...
<CodeGroup>
  ...
  def daily_price_check():
      with client.Session() as session:
          session.execute(actions.Goto(url="https://competitor.com/pricing"))
...
<CodeGroup>
  ...
  def process_order(order_id: str):
      with client.Session() as session:
          session.execute(actions.Goto(url=f"https://admin.example.com/orders/{order_id}"))
          session.execute(actions.Click(selector="button.process"))
...
<CodeGroup>
  ...
  def get_stock_price(symbol: str):
      with client.Session() as session:
          session.execute(actions.Goto(url=f"https://finance.example.com/{symbol}"))
...
<CodeGroup>
  ...
  def check_availability(product_url: str):
      with client.Session() as session:
          session.execute(actions.Goto(url=product_url))
...
<CardGroup cols={2}>
  ...
  <Card title="Scheduling" icon="clock" href="/concepts/workflows">
    Schedule recurring executions
  </Card>
</CardGroup>
